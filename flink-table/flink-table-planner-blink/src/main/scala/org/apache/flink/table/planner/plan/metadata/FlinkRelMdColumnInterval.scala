      ...
      if (index < groupSet.length) {
        ...
        val aggCall = aggregate match {
          ...
          case agg: BatchExecWindowAggregateBase if agg.getAggCallList.length > aggCallIndex =>
            agg.getAggCallList(aggCallIndex)
          case agg: TableAggregate if agg.getAggCallList.length > aggCallIndex =>
            agg.getAggCallList.get(aggCallIndex)
          case agg: StreamExecGroupTableAggregate if agg.aggCalls.length > aggCallIndex =>
            agg.aggCalls.get(aggCallIndex)
          case _ => null
        }
